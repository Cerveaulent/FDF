...
/*   Updated: 2019/10/01 20:57:27 by eschnell    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static t_lpoints	apply_scale(t_lpoints pt, t_hook *m_hook)
{
	...
	result.x = round(pt.x * (m_hook->map_info->map_scale + \
		m_hook->map_info->map_scale));
	result.y = round(pt.y * (m_hook->map_info->map_scale + \
		m_hook->map_info->map_scale));
...
int					print_map(t_hook *m_hook)
{
	...
	while (i < m_hook->map_info->map_size)
	{
		if (((i + 1) % m_hook->map_info->nb_col))
			...
			apply_scale(m_hook->tab_pts[i + 1], m_hook), m_hook->rdr);
		if (i + m_hook->map_info->nb_col < m_hook->map_info->map_size)
			main_bresenham(apply_scale(m_hook->tab_pts[i], m_hook),
			apply_scale(m_hook->tab_pts[i + m_hook->map_info->nb_col], m_hook),
				m_hook->rdr);
		i++;
	}
