...
/*   Updated: 2019/09/13 16:42:30 by ccantin     ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
{
	while (p_a.x  <= p_b.x && (p_a.y * rdr->r_width + p_a.x < rdr->r_width * rdr->r_height))
	{
		if (p_a.x < rdr->r_width && p_a.y < rdr->r_height && p_a.x >= 0 && p_a.y >= 0)
			rdr->img_data[p_a.y * rdr->r_width + p_a.x] = p_a.color;
		p_a.x++;
	}
}
...
static void draw_ver_line(t_lpoints p_b, t_lpoints p_a, t_renderer *rdr)
{
	if (p_a.color != p_b.color)
		p_b.color = p_a.color;
	while (p_a.y <= p_b.y && (p_a.y * rdr->r_width + p_a.x < rdr->r_width * rdr->r_height))
	{
		if (p_a.x < rdr->r_width && p_a.y < rdr->r_height && p_a.x >= 0 && p_a.y >= 0)
			rdr->img_data[p_a.y * rdr->r_width + p_a.x] = p_a.color;
		p_a.y++;
